...
LAYERS_MEDIA = MEDIA_BASE / "layers"
COVERS_MEDIA = MEDIA_BASE / "covers"
LAYERS_MEDIA.mkdir(parents=True, exist_ok=True)
COVERS_MEDIA.mkdir(parents=True, exist_ok=True)
...
class Database:
    ...
    async def get_layers(self, offset=0, limit=10):
        async with self.pool.acquire() as conn:
            total = await conn.fetchval("SELECT COUNT(*) FROM mattress_layers")
            rows = await conn.fetch(
                "SELECT id, code, name, description FROM mattress_layers ORDER BY code OFFSET $1 LIMIT $2",
                offset, limit
            )
    ...
    async def create_layer(self, code: str, name: str, description: str = "") -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO mattress_layers (code, name, description) VALUES ($1, $2, $3) RETURNING id",
    ...
    async def update_layer(self, layer_id: int, code: str, name: str, description: str):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE mattress_layers SET code = $1, name = $2, description = $3 WHERE id = $4",
    ...
    async def delete_layer(self, layer_id: int) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute("DELETE FROM mattress_layers_prices WHERE layer_id = $1", layer_id)
                await conn.execute("DELETE FROM mattress_layer_main_features WHERE layer_id = $1", layer_id)
                await conn.execute("DELETE FROM mattress_layer_extra_features WHERE layer_id = $1", layer_id)
                result = await conn.execute("DELETE FROM mattress_layers WHERE id = $1", layer_id)
                return result != "DELETE 0"

    async def is_layer_code_unique(self, code: str, exclude_id: Optional[int] = None) -> bool:
        async with self.pool.acquire() as conn:
            if exclude_id:
                row = await conn.fetchrow("SELECT id FROM mattress_layers WHERE code = $1 AND id != $2", code, exclude_id)
            else:
                row = await conn.fetchrow("SELECT id FROM mattress_layers WHERE code = $1", code)
            return row is None

    # ---------- Чехлы ----------
    async def get_covers(self, offset=0, limit=10):
        async with self.pool.acquire() as conn:
            total = await conn.fetchval("SELECT COUNT(*) FROM mattress_cover")
            rows = await conn.fetch(
                "SELECT id, code, name, description FROM mattress_cover ORDER BY code OFFSET $1 LIMIT $2",
                offset, limit
            )
            return [dict(r) for r in rows], total
    ...
    async def create_cover(self, code: str, name: str, description: str = "") -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO mattress_cover (code, name, description) VALUES ($1, $2, $3) RETURNING id",
                code, name, description
            )
    ...
    async def update_cover(self, cover_id: int, code: str, name: str, description: str):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE mattress_cover SET code = $1, name = $2, description = $3 WHERE id = $4",
    ...
    async def delete_cover(self, cover_id: int) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute("DELETE FROM mattress_cover_prices WHERE cover_id = $1", cover_id)
                result = await conn.execute("DELETE FROM mattress_cover WHERE id = $1", cover_id)
                return result != "DELETE 0"

    async def is_cover_code_unique(self, code: str, exclude_id: Optional[int] = None) -> bool:
        async with self.pool.acquire() as conn:
            if exclude_id:
                row = await conn.fetchrow("SELECT id FROM mattress_cover WHERE code = $1 AND id != $2", code, exclude_id)
            else:
                row = await conn.fetchrow("SELECT id FROM mattress_cover WHERE code = $1", code)
            return row is None

    # ---------- Размеры ----------
    async def get_sizes(self, offset=0, limit=10):
        async with self.pool.acquire() as conn:
            total = await conn.fetchval("SELECT COUNT(*) FROM mattress_sizes")
            rows = await conn.fetch(
                "SELECT id, name FROM mattress_sizes ORDER BY name OFFSET $1 LIMIT $2",
                offset, limit
            )
            return [dict(r) for r in rows], total
    ...
    async def get_size_by_id(self, size_id: int):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT id, name FROM mattress_sizes WHERE id = $1", size_id)
            return dict(row) if row else None

    async def get_all_sizes(self):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch("SELECT id, name FROM mattress_sizes ORDER BY name")
            return [dict(r) for r in rows]

    async def create_size(self, size_name: str) -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO mattress_sizes (name) VALUES ($1) RETURNING id",
                size_name
            )
    ...
    async def update_size(self, size_id: int, size_name: str):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE mattress_sizes SET name = $1 WHERE id = $2",
                size_name, size_id
            )
    ...
    async def delete_size(self, size_id: int) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute("DELETE FROM mattress_layers_prices WHERE size_id = $1", size_id)
                await conn.execute("DELETE FROM mattress_cover_prices WHERE size_id = $1", size_id)
                result = await conn.execute("DELETE FROM mattress_sizes WHERE id = $1", size_id)
                return result != "DELETE 0"

    async def is_size_unique(self, size_name: str, exclude_id: Optional[int] = None) -> bool:
        async with self.pool.acquire() as conn:
            if exclude_id:
                row = await conn.fetchrow("SELECT id FROM mattress_sizes WHERE name = $1 AND id != $2", size_name, exclude_id)
            else:
                row = await conn.fetchrow("SELECT id FROM mattress_sizes WHERE name = $1", size_name)
            return row is None
    ...
    async def get_layer_prices(self, layer_id: int):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                "FROM mattress_layers_prices lp "
                "JOIN mattress_sizes s ON lp.size_id = s.id "
                "WHERE lp.layer_id = $1 "
                "ORDER BY s.name",
                layer_id
            )
    ...
    async def set_layer_price(self, layer_id: int, size_id: int, price: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO mattress_layers_prices (layer_id, size_id, price) VALUES ($1, $2, $3) "
    ...
    async def delete_layer_price(self, price_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute("DELETE FROM mattress_layers_prices WHERE id = $1", price_id)
    ...
    async def get_cover_prices(self, cover_id: int):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                "FROM mattress_cover_prices cp "
                "JOIN mattress_sizes s ON cp.size_id = s.id "
                "WHERE cp.cover_id = $1 "
                "ORDER BY s.name",
                cover_id
            )
    ...
    async def set_cover_price(self, cover_id: int, size_id: int, price: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO mattress_cover_prices (cover_id, size_id, price) VALUES ($1, $2, $3) "
    ...
    async def delete_cover_price(self, price_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute("DELETE FROM mattress_cover_prices WHERE id = $1", price_id)

    # ---------- Основные характеристики слоёв (справочник) ----------
    async def get_layer_main_feature_types(self, offset=0, limit=10):
        async with self.pool.acquire() as conn:
            total = await conn.fetchval("SELECT COUNT(*) FROM mattress_layer_main_features_types")
            rows = await conn.fetch(
                "SELECT id, name FROM mattress_layer_main_features_types ORDER BY name OFFSET $1 LIMIT $2",
                offset, limit
            )
            return [dict(r) for r in rows], total
    ...
    async def create_layer_main_feature_type(self, name: str) -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO mattress_layer_main_features_types (name) VALUES ($1) RETURNING id",
                name
            )

    async def update_layer_main_feature_type(self, feat_id: int, new_name: str):
        async with self.pool.acquire() as conn:
            await conn.execute("UPDATE mattress_layer_main_features_types SET name = $1 WHERE id = $2", new_name, feat_id)

    async def delete_layer_main_feature_type(self, feat_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute("DELETE FROM mattress_layer_main_features_types WHERE id = $1", feat_id)
    ...
    async def get_layer_main_features(self, layer_id: int):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                "FROM mattress_layer_main_features_types mft "
                "LEFT JOIN mattress_layer_main_features pmf ON mft.id = pmf.feature_id AND pmf.layer_id = $1 "
                "ORDER BY mft.name",
                layer_id
            )
            return [dict(r) for r in rows]
    ...
    async def set_layer_main_feature(self, layer_id: int, feature_id: int, value: str):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO mattress_layer_main_features (layer_id, feature_id, value) "
                "VALUES ($1, $2, $3) "
    ...
    async def get_layer_extra_features(self, layer_id: int):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT id, name, value FROM mattress_layer_extra_features WHERE layer_id = $1 ORDER BY name",
                layer_id
            )
            return [dict(r) for r in rows]
    ...
    async def add_layer_extra_feature(self, layer_id: int, name: str, value: str) -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO mattress_layer_extra_features (layer_id, name, value) VALUES ($1, $2, $3) RETURNING id",
                layer_id, name, value
            )
    ...
    async def update_layer_extra_feature(self, feat_id: int, name: str = None, value: str = None):
        async with self.pool.acquire() as conn:
            if name is not None and value is not None:
                await conn.execute(
                    "UPDATE mattress_layer_extra_features SET name = $1, value = $2 WHERE id = $3",
                    name, value, feat_id
                )
            elif name is not None:
                await conn.execute("UPDATE mattress_layer_extra_features SET name = $1 WHERE id = $2", name, feat_id)
            elif value is not None:
                await conn.execute("UPDATE mattress_layer_extra_features SET value = $1 WHERE id = $2", value, feat_id)

    async def delete_layer_extra_feature(self, feat_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute("DELETE FROM mattress_layer_extra_features WHERE id = $1", feat_id)
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "🔧 *Главное меню управления матрацами*"
    if update.callback_query:
...
# ------------------------- Работа с фото слоёв и чехлов -------------------------
def get_layer_photo_path(layer_code: str) -> Path:
    return LAYERS_MEDIA / f"{layer_code}.webp"

def get_cover_photo_path(cover_code: str) -> Path:
    return COVERS_MEDIA / f"{cover_code}.webp"
...
async def save_layer_photo(file, layer_code: str) -> bool:
    try:
        ...
        if img.mode in ('RGBA', 'LA', 'P'):
            ...
            img = rgb_img
        filepath = get_layer_photo_path(layer_code)
        img.save(filepath, 'WEBP', quality=85)
        return True
    except Exception as e:
        logger.error(f"Ошибка сохранения фото слоя: {e}")
        return False

async def delete_layer_photo(layer_code: str):
    path = get_layer_photo_path(layer_code)
    if path.exists():
        path.unlink()

async def move_layer_photo(old_code: str, new_code: str):
    old_path = get_layer_photo_path(old_code)
    new_path = get_layer_photo_path(new_code)
    if old_path.exists():
        if new_path.exists():
            new_path.unlink()
        old_path.rename(new_path)

async def save_cover_photo(file, cover_code: str) -> bool:
    try:
        photo_bytes = await file.download_as_bytearray()
        img = Image.open(io.BytesIO(photo_bytes))
        if img.mode in ('RGBA', 'LA', 'P'):
            ...
            rgb_img.paste(img, mask=img.split()[-1] if img.mode == 'RGBA' else None)
            img = rgb_img
        filepath = get_cover_photo_path(cover_code)
        img.save(filepath, 'WEBP', quality=85)
        return True
    except Exception as e:
        logger.error(f"Ошибка сохранения фото чехла: {e}")
        return False

async def delete_cover_photo(cover_code: str):
    path = get_cover_photo_path(cover_code)
    if path.exists():
        path.unlink()

async def move_cover_photo(old_code: str, new_code: str):
    old_path = get_cover_photo_path(old_code)
    new_path = get_cover_photo_path(new_code)
    if old_path.exists():
        if new_path.exists():
            new_path.unlink()
        old_path.rename(new_path)
...
async def layer_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🖼️ Посмотреть фото", callback_data=f"layer_view_photo_{layer_id}_{page}")],
        [InlineKeyboardButton("◀ К списку слоёв", callback_data=f"layers_page_{page}")],
        [InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu")],
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

async def layer_view_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    parts = query.data.split("_")
    ...
    if not layer:
        await query.edit_message_text("❌ Слой не найден.")
        return
    path = get_layer_photo_path(layer['code'])
    if path.exists():
...
async def layer_edit_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📸 Управление фото", callback_data="layer_photo_manage")],
...
async def layer_update_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await move_layer_photo(old_code, new_code)
...
async def layer_photo_manage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if not layer:
        await query.edit_message_text("❌ Слой не найден.")
        return ConversationHandler.END
    context.user_data["photo_layer_id"] = layer_id
    path = get_layer_photo_path(layer['code'])
    has_photo = path.exists()
    text = f"📸 *Управление фото слоя {layer['code']}*\n\n"
    text += "✅ Фото есть" if has_photo else "❌ Фото отсутствует"
    keyboard = []
    if has_photo:
        keyboard.append([InlineKeyboardButton("🖼️ Посмотреть фото", callback_data="layer_photo_view")])
        keyboard.append([InlineKeyboardButton("🗑️ Удалить фото", callback_data="layer_photo_delete")])
    keyboard.append([InlineKeyboardButton("➕ Добавить / заменить фото", callback_data="layer_photo_add")])
    keyboard.append([InlineKeyboardButton("◀ Назад к редактированию", callback_data=f"layer_edit_back")])
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    return LAYER_PHOTO_MANAGE

async def layer_photo_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    layer_id = context.user_data.get("photo_layer_id")
    if not layer_id:
    ...
    if layer:
        ...
        if path.exists():
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Фото отсутствует.")
    await layer_photo_manage(update, context)

async def layer_photo_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("Отправьте новое фото для слоя (одно изображение):", reply_markup=CANCEL_KEYBOARD)
    return LAYER_PHOTO_ADD

async def layer_photo_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        layer_id = context.user_data.get("photo_layer_id")
        if not layer_id:
            await update.message.reply_text("❌ Ошибка: слой не найден.")
            return ConversationHandler.END
        layer = await db.get_layer_by_id(layer_id)
        if not layer:
...
async def layer_photo_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    layer_id = context.user_data.get("photo_layer_id")
    if not layer_id:
...
async def cover_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🖼️ Посмотреть фото", callback_data=f"cover_view_photo_{cover_id}_{page}")],
        [InlineKeyboardButton("◀ К списку чехлов", callback_data=f"covers_page_{page}")],
        [InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu")],
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

async def cover_view_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    parts = query.data.split("_")
    cover_id = int(parts[3])
    page = int(parts[4])
    cover = await db.get_cover_by_id(cover_id)
    if not cover:
        await query.edit_message_text("❌ Чехол не найден.")
        return
    path = get_cover_photo_path(cover['code'])
    if path.exists():
        with open(path, 'rb') as f:
            await context.bot.send_photo(chat_id=update.effective_chat.id, photo=f)
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ У чехла нет фото.")
    await cover_details(update, context)
...
async def cover_edit_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📸 Управление фото", callback_data="cover_photo_manage")],
...
async def cover_update_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await move_cover_photo(old_code, new_code)
    await update.message.reply_text("✅ Код обновлён, фото переименовано.")
    await show_covers(update, context, context.user_data["edit_cover_page"])
    return ConversationHandler.END
...
async def cover_photo_manage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    cover_id = context.user_data.get("edit_cover_id")
    if not cover_id:
        await query.edit_message_text("❌ Ошибка: чехол не выбран.")
        return ConversationHandler.END
    cover = await db.get_cover_by_id(cover_id)
    if not cover:
        await query.edit_message_text("❌ Чехол не найден.")
        return ConversationHandler.END
    context.user_data["photo_cover_id"] = cover_id
    path = get_cover_photo_path(cover['code'])
    has_photo = path.exists()
    text = f"📸 *Управление фото чехла {cover['code']}*\n\n"
    text += "✅ Фото есть" if has_photo else "❌ Фото отсутствует"
    keyboard = []
    if has_photo:
        keyboard.append([InlineKeyboardButton("🖼️ Посмотреть фото", callback_data="cover_photo_view")])
        keyboard.append([InlineKeyboardButton("🗑️ Удалить фото", callback_data="cover_photo_delete")])
    keyboard.append([InlineKeyboardButton("➕ Добавить / заменить фото", callback_data="cover_photo_add")])
    keyboard.append([InlineKeyboardButton("◀ Назад к редактированию", callback_data=f"cover_edit_back")])
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    return COVER_PHOTO_MANAGE

async def cover_photo_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    cover_id = context.user_data.get("photo_cover_id")
    if not cover_id:
        await query.edit_message_text("❌ Ошибка.")
        return COVER_PHOTO_MANAGE
    cover = await db.get_cover_by_id(cover_id)
    if cover:
        ...
        if path.exists():
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Фото отсутствует.")
    await cover_photo_manage(update, context)

async def cover_photo_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("Отправьте новое фото для чехла (одно изображение):", reply_markup=CANCEL_KEYBOARD)
    return COVER_PHOTO_ADD

async def cover_photo_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.photo:
        await update.message.reply_text("❌ Отправьте фото.", reply_markup=CANCEL_KEYBOARD)
        return COVER_PHOTO_ADD
    try:
        ...
        cover_id = context.user_data.get("photo_cover_id")
        if not cover_id:
            await update.message.reply_text("❌ Ошибка: чехол не найден.")
            return ConversationHandler.END
        cover = await db.get_cover_by_id(cover_id)
        if not cover:
            await update.message.reply_text("❌ Чехол не найден.")
            return ConversationHandler.END
        await delete_cover_photo(cover['code'])
        success = await save_cover_photo(photo_file, cover['code'])
        if success:
            await update.message.reply_text("✅ Фото сохранено.")
        else:
            await update.message.reply_text("❌ Ошибка сохранения фото.")
    except Exception as e:
        logger.error(f"Ошибка загрузки фото чехла: {e}")
        await update.message.reply_text("❌ Не удалось скачать фото.")
    await cover_photo_manage(update, context)
    return ConversationHandler.END

async def cover_photo_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    cover_id = context.user_data.get("photo_cover_id")
    if not cover_id:
...
async def cover_edit_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    cover_id = context.user_data.get("edit_cover_id")
    page = context.user_data.get("edit_cover_page", 0)
    if cover_id:
        await show_cover_edit(update, context, cover_id, page)
    else:
        await query.edit_message_text("❌ Ошибка: чехол не найден.")
    return ConversationHandler.END

async def show_cover_edit(update, context, cover_id, page):
    cover = await db.get_cover_by_id(cover_id)
    if not cover:
        await update.message.reply_text("❌ Чехол не найден.")
        return
...
async def size_add_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("Введите *название* размера (например, 140x70):",
                                                  parse_mode="Markdown", reply_markup=CANCEL_KEYBOARD)
...
def register_handlers(app: Application):
    ...
    app.add_handler(CallbackQueryHandler(layer_view_photo, pattern="^layer_view_photo_\\d+_\\d+$"))
    ...
    layer_edit_conv = ConversationHandler(
        ...
        states={
            LAYER_EDIT_SELECT: [
                ...
                CallbackQueryHandler(layer_photo_manage, pattern="^layer_photo_manage$"),
            ...
            LAYER_PHOTO_MANAGE: [
                ...
                CallbackQueryHandler(layer_photo_delete, pattern="^layer_photo_delete$"),
                CallbackQueryHandler(layer_edit_back, pattern="^layer_edit_back$"),
            ],
    ...
    app.add_handler(CallbackQueryHandler(cover_view_photo, pattern="^cover_view_photo_\\d+_\\d+$"))
    ...
    cover_edit_conv = ConversationHandler(
        ...
        states={
            COVER_EDIT_SELECT: [
                ...
                CallbackQueryHandler(cover_photo_manage, pattern="^cover_photo_manage$"),
                CallbackQueryHandler(cover_edit_delete, pattern="^cover_edit_delete$"),
                CallbackQueryHandler(cover_edit_back, pattern="^cover_edit_back$"),
            ...
            COVER_PHOTO_MANAGE: [
                ...
                CallbackQueryHandler(cover_photo_delete, pattern="^cover_photo_delete$"),
                CallbackQueryHandler(cover_edit_back, pattern="^cover_edit_back$"),
            ],
            COVER_PHOTO_ADD: [MessageHandler(filters.PHOTO, cover_photo_save)],
        },
        fallbacks=[CommandHandler("cancel", cancel), CallbackQueryHandler(cancel, pattern="^main_menu$")],
        per_message=False,
    )
...
async def main():
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("""
            -- Таблицы для характеристик слоёв (если не существуют)
            ...
            CREATE TABLE IF NOT EXISTS mattress_layer_main_features (
                id SERIAL PRIMARY KEY,
                layer_id INTEGER REFERENCES mattress_layers(id) ON DELETE CASCADE,
                feature_id INTEGER REFERENCES mattress_layer_main_features_types(id) ON DELETE CASCADE,
                value TEXT NOT NULL,
        ...
        """)
        logger.info("Таблицы характеристик созданы/проверены.")
